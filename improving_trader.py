...
class Trader:
    ...
    # L1 imbalance had +0.62 correlation with next mid return on round-0 data.
    # Shift reservation up to ±1.5 ticks based on imbalance.
    EMERALDS_IMB_COEF = 2.5
    # When |imbalance| exceeds this, post smaller size on the threatened side.
    EMERALDS_IMB_THRESH = 0.20
    ...
    def _trade_emeralds(self, depth: OrderDepth, position: int) -> List[Order]:
        ...
        # L1 imbalance: +1 = bid heavy (price likely up), -1 = ask heavy (price likely down).
        imb = self._l1_imbalance(bid_orders, ask_orders)

        # Reservation tilts toward the side imbalance is pointing.
        reservation = (
            self.EMERALDS_FAIR
            - self.EMERALDS_INV_SKEW * position
            + self.EMERALDS_IMB_COEF * imb
        )
        ...
        # Step 2: passive quotes around (skewed) reservation.
        ...
        # Asymmetric sizing: when book leans hard one way, post smaller on the
        # side that's about to get run over (defensive against adverse selection).
        bid_size, ask_size = buy_cap, sell_cap
        if imb > self.EMERALDS_IMB_THRESH:
            ask_size = max(1, sell_cap // 2)
        elif imb < -self.EMERALDS_IMB_THRESH:
            bid_size = max(1, buy_cap // 2)

        if bid_size > 0:
            orders.append(Order("EMERALDS", bid_px, bid_size))
        if ask_size > 0:
            orders.append(Order("EMERALDS", ask_px, -ask_size))
